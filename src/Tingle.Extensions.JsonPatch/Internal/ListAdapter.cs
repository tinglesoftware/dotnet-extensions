...
namespace Tingle.Extensions.JsonPatch.Internal
{
    ...
    public class ListAdapter : IAdapter
    {
            ...
            out string? errorMessage)
        {
            ...
            if (!comparer.Equals(JsonDocument.Parse(JsonSerializer.Serialize(currentValue)).RootElement, JsonDocument.Parse(JsonSerializer.Serialize(convertedValue)).RootElement))
            {
        ...
        protected virtual bool TryConvertValue(
            ...
            out string? errorMessage)
        {
            if (listTypeArgument is null)
            {
                throw new ArgumentNullException(nameof(listTypeArgument));
            }
