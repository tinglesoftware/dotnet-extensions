...
public enum FirebaseMessageAndroidPriority
{
    ...
    /// FCM attempts to deliver high priority messages immediately, allowing the FCM service to wake
    /// a sleeping device when possible and open a network connection to your app server.
    /// Apps with instant messaging, chat, or voice call alerts, for example, generally need to open
    /// a network connection and make sure FCM delivers the message to the device without delay.
